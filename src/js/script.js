...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const btnTrigger = document.querySelector("#js-hamburger .btn-trigger");
    
    // 開閉対象のメニュー要素群を取得
    const drower = document.getElementById("js-drower");
    const mask = document.getElementById("js-mask");
    const naviMenu = document.getElementById("js-navi__menu");
    ...
    if (hamburger && btnTrigger) {
        ...
        hamburger.addEventListener("click", function () {
            ...
            if (!btnTrigger.classList.contains("active")) {
                ...
                document.querySelectorAll(".l-global-menu-hamburger.active").forEach(item => {
                    item.classList.remove("active");
                });

                // アコーディオンコンテンツ (.l-global-menu-hamburger__aco) を全て非表示にする
                document.querySelectorAll(".l-global-menu-hamburger__aco").forEach(content => {
...
// 追従ヘッダーのカラー変更
document.addEventListener("DOMContentLoaded", function () {
  const header = document.querySelector(".l-header-back");
  const navs = document.querySelectorAll(".l-global-menu-items, .l-hamburger__bar");
  const beforeElementParent = document.querySelector(".l-global-menu-drop__txt"); // 親要素を選択
  const target = document.querySelector(".target");
  const changePoint = target.offsetTop;
...
const targetSection = document.querySelector('.p-top__feature');
const absoluteChildrenInSection = targetSection.querySelectorAll(
  '.p-top__feature-01, ' // p-top__feature-01自体がabsoluteなら含める
  + '.p-top__feature-02, ' // p-top__feature-02自体がabsoluteなら含める
  + '.c-overlap-textbox__left, '
  + '.c-overlap-textbox__blue-bg-left, '
  + '.c-overlap-textbox__right, '
  + '.c-overlap-textbox__blue-bg-right'
  );

function setSectionHeight() {
    let maxHeight = 0;
    absoluteChildrenInSection.forEach(child => {
    ...
    targetSection.style.minHeight = `${maxHeight}px`; // min-heightで設定
}
